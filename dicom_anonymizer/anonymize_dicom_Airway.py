...
def anonymize_dicom(ds,analysis_vars):
...
# Tags to save in Excel for Data Statistics in the Future
    anal_tags = [\
                 Tag(0x0008, 0x0050), # 1 Accession Number 
                 Tag(0x0008, 0x0033), # 1 Image Time
                 Tag(0x0008, 0x1010), # 2 Station Name 
                 Tag(0x0008, 0x0020), # 3 Study Date 
                 Tag(0x0008, 0x103E), # 4 Series Description 
                 Tag(0x0010, 0x0040), # 5 Patient's Sex 
                 Tag(0x0010, 0x1010), # 6 Patient's Age  
                 Tag(0x0010, 0x2160), # 7 Ethnic Group 
                 Tag(0x0018, 0x1030), # 8 Protocol Name
                 Tag(0x0018, 0x1050), # 9 Spatial Resolution
                 Tag(0x0028, 0x0010), # 10 Rows
                 Tag(0x0028, 0x0011), # 11 Columns
                 Tag(0x0018, 0x1110), # 12 Source To Detector
                 Tag(0x0018, 0x1111)] # 13 Source to Patient
    ...
    for t in anal_tags:
        if t in keys:
            values = ds[t].value
            if(type(values) == dicom.valuerep.DSfloat):
                values = values.real
        else:
            values='NA'  
        #print('tag',t, ' value ', values)
        analysis_vars.append(values)
